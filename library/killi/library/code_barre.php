...
// *****                        CLASSE code_barre                          *****
// *****************************************************************************
// ***** code_barre()               : Constructeur et ré-initialisation
// ***** 
// *****************************************************************************
// ***** Méthodes Publiques :
// *****************************************************************************
...
//	                $encodedString .= "32"; //Stop
			...
			if($this->HR!='' && $this->HR!='AUTO'){
				$minTextWidth=(strlen($this->HR)*7)+($this->CALMZONE*2);
			}else{
				$minTextWidth=0;
			}
			
			$this->CODEWIDTH = max( $this->CODEWIDTH, $nb_elem );
			$this->WIDTH = max( $this->WIDTH, $this->CODEWIDTH + ($this->CALMZONE*2),$minTextWidth );
			...
			*/
			...
			$this->IH = imagecreatetruecolor($this->WIDTH, $this->HEIGHT);
		...
		*/
		...
		/**
		* Renvois l'image en base64 avec l'entete src
		*/
		
		function showInlineSrc($directOutput=false)
		{
			$this->checkCode();
			$this->encode();
...
*/
if (strpos($_SERVER['PHP_SELF'], 'code_barre.php'))
{
	$objCode = new code_barre() ;
	
	$height=(!empty($_REQUEST['hauteur']))?$_REQUEST['hauteur']:40;
	$width=(!empty($_REQUEST['largeur']))?$_REQUEST['largeur']:0;
	$objCode->setSize($height, $width);
	
	$code=(!empty($_REQUEST['code']))?$_REQUEST['code']:'1234567890';
	
	if(!empty($_REQUEST['format']))
	{
		$objCode->setFiletype($_REQUEST['format']);
	}
	if(!empty($_REQUEST['type']))
	{
		$objCode->setType(strtoupper($_REQUEST['type']));
	}
	if(isset($_REQUEST['lisible']))
	{
		if(empty($_REQUEST['lisible']) || $_REQUEST['lisible']=='false' || $_REQUEST['lisible']===false || $_REQUEST['lisible']=='N')
		{
			$objCode->setText('');
		}
	}
	if(!isset($_REQUEST['voireTypeCode']))
	{
	...
	if(!empty($_REQUEST['color']))
	{
		...
		{
			$objCode->setColors($_REQUEST['color'], $_REQUEST['bgcolor']);
		}else{
			$objCode->setColors($_REQUEST['color']);
		}
	}
	if(!empty($_REQUEST['type']))
	{
		...
		$objCode->setCode(strtoupper($code));
	}
	if(!empty($_REQUEST['encode']))
	{
		echo($objCode->showInlineSrc());
	}else{
