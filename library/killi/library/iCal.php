<?php
// Ajout de la prise en charge de la description et remplacement de text par summary
// Class permettant de formatter les donnes provenant dun array ou XML au format iCal
...
class ICalExporter {
	...
	function simpleParseXML($string) {
		...
		for($i=0;$i<sizeof($xmlobj->event);$i++) {
			$a[$i] = array(
				...
				'summary' => (string)$xmlobj->event[$i]->summary,
				...
				'description' => (string)$xmlobj->event[$i]->description,
			);
		}
		return $a;
	}
	...
	function toICal($h) {
		...
		foreach($h as $h_v)
		{
			if($h_v['event_pid'] != 0 and $h_v['rec_type'] == "") {
				$str .= "BEGIN:VEVENT\n";
				$str .= "DTSTART:".$this->getTime($h_v['start_date'])."\n";
				$str .= "DTEND:".$this->getTime($h_v['end_date'])."\n";
				$str .= "RECURRENCE-ID:".date("Ymd\THis",$h_v['event_length'])."\n";
				$str .= "UID:".$h_v['event_pid']."\n";
				$str .= "SUMMARY:".$this->formatString($h_v['summary'])."\n";
				$str .= "DESCRIPTION:".$this->formatString($h_v['description'])."\n";
				$str .= "END:VEVENT\n";
			}
			elseif($h_v['rec_type'] != "" and $h_v['event_pid'] == 0) {
				$str .= "BEGIN:VEVENT\n";
				$str .= "DTSTART:".$this->getStartTimeEvent($h_v)."\n";
				$str .= "DTEND:".$this->getEndTimeEvent($h_v)."\n";
				$str .= "RRULE:".$this->getRrule($h_v)."\n";
					$exdate = $this->getExdate($h_v['event_id'], $h_v);
						if($exdate != 0) { $str .= "EXDATE:".$exdate."\n"; }
				$str .= "UID:".$h_v['event_id']."\n";
				$str .= "SUMMARY:".$this->formatString($h_v['summary'])."\n";
				$str .= "DESCRIPTION:".$this->formatString($h_v['description'])."\n";
				$str .= "END:VEVENT\n";
			}
	...
	function toXML($str) {
		...
		for($i=1;$i<=sizeof($arr);$i++) {
			...
			$str .= '<summary><![CDATA['.$arr[$i]['summary'].']]></summary>';
			...
			$str .= '<description><![CDATA['.$arr[$i]['summary'].']]></description>';
			$str .= '</event>';
		}
		$str .= '</data>';
		return $str;
